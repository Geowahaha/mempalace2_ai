...
class Settings(BaseSettings):
    ...
    # --- Backtest learning supervisor (Hermes-style continuous research lane) ---
    backtest_learning_enabled: bool = Field(
        default=False,
        validation_alias="BACKTEST_LEARNING_ENABLED",
        description="Periodically run isolated backtests and feed safe adaptive policy hints into the live loop.",
    )
    backtest_learning_interval_sec: int = Field(
        default=21600,
        validation_alias="BACKTEST_LEARNING_INTERVAL_SEC",
        ge=300,
        le=604800,
        description="Seconds between successful backtest-learning cycles.",
    )
    backtest_learning_failure_backoff_sec: int = Field(
        default=1800,
        validation_alias="BACKTEST_LEARNING_FAILURE_BACKOFF_SEC",
        ge=60,
        le=86400,
        description="Retry delay after a failed backtest-learning cycle.",
    )
    backtest_learning_lookback_days: int = Field(
        default=7,
        validation_alias="BACKTEST_LEARNING_LOOKBACK_DAYS",
        ge=2,
        le=90,
        description="Local-day lookback window for each research replay.",
    )
    backtest_learning_end_offset_days: int = Field(
        default=1,
        validation_alias="BACKTEST_LEARNING_END_OFFSET_DAYS",
        ge=0,
        le=7,
        description="End window offset from today to avoid unstable in-flight sessions.",
    )
    backtest_learning_timezone: str = Field(
    ...
    backtest_learning_dexter_root: Path = Field(
        default=Path("../dexter_pro_v3_fixed/dexter_pro_v3_fixed"),
        validation_alias="BACKTEST_LEARNING_DEXTER_ROOT",
        description="Read-only Dexter root used to source historical replay data.",
    )
    backtest_learning_output_root: Path = Field(
        default=Path("./data/backtests_auto"),
        validation_alias="BACKTEST_LEARNING_OUTPUT_ROOT",
        description="Artifact directory for automatic research backtests.",
    )
    backtest_learning_timeout_sec: int = Field(
        default=1800,
        validation_alias="BACKTEST_LEARNING_TIMEOUT_SEC",
        ge=120,
        le=21600,
        description="Timeout budget for one backtest-learning replay.",
    )
    backtest_learning_policy_apply_enabled: bool = Field(
        default=True,
        validation_alias="BACKTEST_LEARNING_POLICY_APPLY_ENABLED",
        description="If true, apply quality-gated adaptive hard-filter overrides into live loop decisions.",
    )
    backtest_learning_policy_max_shift: float = Field(
        default=0.06,
        validation_alias="BACKTEST_LEARNING_POLICY_MAX_SHIFT",
        ge=0.0,
        le=0.25,
        description="Maximum per-cycle threshold shift (safety cap) for adaptive hard-filter policy.",
    )
    backtest_learning_min_closed_trades: int = Field(
        default=12,
        validation_alias="BACKTEST_LEARNING_MIN_CLOSED_TRADES",
        ge=2,
        le=2000,
        description="Minimum replay closed trades required before relaxing filters.",
    )
    backtest_learning_min_win_rate: float = Field(
        default=0.5,
        validation_alias="BACKTEST_LEARNING_MIN_WIN_RATE",
        ge=0.0,
        le=1.0,
        description="Minimum replay win rate required for relax-mode updates.",
    )
    backtest_learning_min_avg_profit: float = Field(
        default=0.0,
        validation_alias="BACKTEST_LEARNING_MIN_AVG_PROFIT",
        description="Minimum replay average profit required for relax-mode updates.",
    )
    backtest_learning_max_drawdown: float = Field(
        default=1.2,
        validation_alias="BACKTEST_LEARNING_MAX_DRAWDOWN",
        ge=0.0,
        description="Maximum replay drawdown allowed before the policy is tightened.",
    )
    backtest_learning_policy_path: Path = Field(
        default=Path("./data/backtest_learning_policy.json"),
        validation_alias="BACKTEST_LEARNING_POLICY_PATH",
        description="Latest adaptive policy artifact generated by the research lane.",
    )
    backtest_learning_state_path: Path = Field(
        default=Path("./data/backtest_learning_state.json"),
        validation_alias="BACKTEST_LEARNING_STATE_PATH",
        description="Supervisor runtime state for scheduling/backoff.",
    )
    backtest_learning_summary_path: Path = Field(
        default=Path("./data/backtest_learning_summary.md"),
        validation_alias="BACKTEST_LEARNING_SUMMARY_PATH",
        description="Human-readable summary of the latest research-policy cycle.",
    )
    ...
    def _normalize_paths(self) -> "Settings":
        ...
        self.backtest_learning_output_root = _resolve_path(self.backtest_learning_output_root, base)
        self.backtest_learning_policy_path = _resolve_path(self.backtest_learning_policy_path, base)
        self.backtest_learning_state_path = _resolve_path(self.backtest_learning_state_path, base)
        self.backtest_learning_summary_path = _resolve_path(self.backtest_learning_summary_path, base)
        ...
        self.backtest_learning_dexter_root = _resolve_external_path(self.backtest_learning_dexter_root)
        return self
