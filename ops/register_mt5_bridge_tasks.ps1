param(
    [string]$TaskPrefix = "Dexter-MT5-Bridge",
...
$defaultBridgeRoot = "D:\dexter_pro_v3_fixed\_refs\OpenClaw-MT5-python-bridge"
...
$taskShim = Join-Path $taskShimDir "mt5_bridge_watchdog.vbs"
...
if ($BridgeRoot -ne $defaultBridgeRoot) {
    $psArgs += @("-BridgeRoot", $BridgeRoot)
}
if ($pythonPath -ne $defaultPythonExe) {
    $psArgs += @("-PythonExe", $pythonPath)
}
...
$startupCmd = Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\Startup\Dexter_MT5_Bridge_Watchdog.cmd"

# Remove stale tasks first to avoid duplicate conflicting definitions.
...
if ($RunAsSystem) {
    ...
    # MT5 bridge generally needs user session (terminal/auth/profile), so use logon trigger.
    $triggers += New-ScheduledTaskTrigger -AtLogOn
}
...
try {
    ...
    # Fallback for Windows builds where repeat trigger registration via CIM is flaky.
    $taskCmd = "`"$wscriptExe`" //B //nologo `"$taskShim`""
    if ($RunAsSystem) {
...
if (Get-ScheduledTask -TaskName $healthName -ErrorAction SilentlyContinue) {
    ...
    schtasks /Run /TN $healthName | Out-Null
}

# Cleanup user-level startup fallback to avoid duplicate watchdog loops.
if (Test-Path $startupCmd) {
    Remove-Item -Path $startupCmd -Force -ErrorAction SilentlyContinue
}
Get-CimInstance Win32_Process -Filter "name='powershell.exe'" |
    Where-Object { [string]($_.CommandLine) -like "*mt5_bridge_guard_loop.ps1*" } |
    ForEach-Object {
        try {
...
Get-ScheduledTask -TaskName $healthName -ErrorAction SilentlyContinue | Get-ScheduledTaskInfo | Select-Object LastRunTime, LastTaskResult, NextRunTime | Format-Table -AutoSize
